<template>
  ...
  @mouseup="end"
  @touchcancel="end">
    <div class="md-swipeout-button-box md-swipeout-button-box-left" :style="leftButtonBoxStyle" v-show="distX >= 0">
      <slot name="left-menu"></slot>
    </div>
    <div class="md-swipeout-button-box" :style="rightButtonBoxStyle" v-show="distX <= 0">
      <slot name="right-menu"></slot>
    </div>
    <div class="md-swipeout-content" :style="styles" @mousedown="onContentClick" @touchstart="onContentClick" ref="content">
...
export default {
  ...
  methods: {
    ...
    onItemClick () {
      if (this.mdAutoCloseOnButtonClick) {
        this._setClose()
      }
    },
    start (ev) {
    ...
    move (ev) {
      ...
      if (this.valid === undefined) {
        if (this.distX > 0 && this.hasLeftMenu === false) {
          ...
          this.valid = false
        } else if (Math.abs(this.distX) > this.mdSensitivity || Math.abs(this.distY) > this.mdSensitivity) {
      ...
      if (this.valid === true) {
        if (this.distX < 0 && this.direction === 'right') {
          const threshold = this.mdThreshold <= 1 ? this.rightMenuWidth * this.mdThreshold : this.mdThreshold
          ...
          const threshold = this.mdThreshold <= 1 ? this.leftMenuWidth * this.mdThreshold : this.mdThreshold
    ...
    setOffset (x, animated = false, force) {
      ...
      if (animated && this.target) {
        this.target && this.target.classList.add('md-swipeout-content-animated')
        var cb = (function (self, target) {
          return function () {
            target.classList.remove('md-swipeout-content-animated')
  ...
  computed: {
    ...
    buttonBoxStyle () {
      return {
        backgroundColor: this.mdUnderlayColor
      }
    },
    leftButtonBoxStyle () {
      let styles = JSON.parse(JSON.stringify(this.buttonBoxStyle))
      if (this.mdTransitionMode === 'follow') {
